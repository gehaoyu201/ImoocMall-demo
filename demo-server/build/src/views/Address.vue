    ...
    <div class="container">
      <div class="checkout-addr">
        ...
        <div class="addr-list-wrap">
          <div class="addr-list">
            <ul>
              <li v-for="(item,index) in addressListFilter" v-bind:class="{'check':checkIndex==index}" @click="checkIndex=index,selAddressId=item.addressId">
                <dl>
                  <dt>{{item.userName}}</dt>
                ...
                <div class="addr-opration addr-del">
                  <a href="javascript:;" class="addr-del-btn" @click="showDelConfirm=true,addressId=item.addressId">
              ...
              <li class="addr-new" @click="showAddConfirm=true">
...
<!--          <a class="btn btn&#45;&#45;m btn&#45;&#45;red">Next</a>-->
          <router-link class="btn btn--m btn--red" v-bind:to="{path:'orderConfirm',query:{'addressId':selAddressId}}">Next</router-link>
        </div>
      </div>
    </div>
  </div>
    <nav-footer></nav-footer>
    <modal v-bind:mdShow="showDelConfirm" @close="showDelConfirm=false">
      <p slot="message">是否确认删除地址？</p>
      <div slot="btnGroup">
        <a href="javascript:;" class="btn btn--m" @click="delAddress">确认</a>
        <a href="javascript:;" class="btn btn--m" @click="showDelConfirm=false">取消</a>
      </div>
    </modal>
    <modal v-bind:mdShow="showAddConfirm" @close="showAddConfirm=false">
      <div slot="message">
        姓名:<input type="text" v-model="userName"><br>
        地址:<input type="text" v-model="streetName"><br>
        邮编:<input type="text" v-model="postCode"><br>
        电话:<input type="text" v-model="tel"><br>
      </div>
        <div slot="btnGroup">
          <a href="javascript:;" class="btn btn--m" @click="addNewAddress">提交</a>
        </div>
    </modal>
  </div>
...
<script>
    ...
    export default {
        ...
        computed:{
            addressListFilter(){
                // return this.addressList.splice(0,3);
        ...
        methods:{
            ...
            setDefault(addressId){
                ...
                axios.post('/users/setDefault',param).then((response)=>{
                    let res = response.data;
                    if(res.status == 0){
                        this.init();
                    }
            ...
            delAddress(){
                ...
                axios.post('/users/delAddress',param).then((response)=>{
                    let res = response.data;
                    if(res.status == 0){
            ...
            addNewAddress(){
                ...
                axios.post('/users/newAdd',param).then((response)=>{
                   let res = response.data;
                   if(res.status == 0){
